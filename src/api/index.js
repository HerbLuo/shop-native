...
import {appUrlBase, serverUrlBase} from '../config'
...
export default {
  ...
  app: {
     ...
     */
    getInitSnippetWeb () {
      return `${this.appBase}/web/js/sub_app_init_snippet.min.js`
    },

    getConsoleQRCodeJs () {
      return `${this.appBase}/web/js/console_qrcode.min.js`
    },

    /**
     * Native端
     * 获取单个App的地址（Web端用router实现，Native端用navigator）
     */
    getNativeAppJsPath (appName) {
      return `${this.appBase}/weex/js/${appName}.js`
    }
  ...
  url: {
     ...
     * rushBuy 抢购
     */
    getRushbuy (page, apagesize) {
      return `${this.base}/app/native/rushbuy/page/${page}/apagesize/${apagesize}/`
    },
     ...
     */
    getProvCityArea () {
      return `${this.base}/address/city-info/province-city-area/`
    },

    /**
     * 获取街道信息
     */
    getStreetByAreaCode (areaCode) {
      return `${this.base}/address/city-info/street/area-code/${areaCode}/`
    },
    ...
    refreshToken (refreshToken) {
      return `${this.base}/oauth/token?grant_type=refresh_token&client_id=any&client_secret=s_any&refresh_token=${refreshToken}`
    },
     ...
     * @param itemVersions
     */
    getItemsByIdsAndVersions (itemIds, itemVersions) {
      let url = `${this.base}/item/?`
      itemIds.forEach(itemId => {
        url += `itemIds=${itemId}&`
      })
      itemVersions.forEach(itemVersion => {
        url += `itemVersions=${itemVersion}&`
      })
      return url.substring(0, url.length - 1)
    },
